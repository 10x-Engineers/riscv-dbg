  ...
  always_comb begin : p_fsm
    ...
    unique case (state_q)
      ...
      dm::Write: begin
        ...
        we  = 1'b1;
        be = be_mask;
        // RISC-V Debug Spec 1.0 #507: autoincrement only on actual bus access
        // For writes: increment after gnt (bus accepted the write)
        // For reads: increment after sbdata_valid_o (read data returned)
        if (gnt) begin
          state_d = dm::WaitWrite;
      ...
      dm::WaitWrite: begin
        if (sbdata_valid_o) begin
          state_d = dm::Idle;
          // autoincrement removed - moved to Write state after gnt (#507)
        end
      end
